...
nurses <- function(n,q,r,d,p,timelim,dbg=F) {
  ...
  simlist$lasttime <- 0 # 上次事件发生的时间，currting 是跳出去的时间/新事件发生时间
  simlist$queue <- NULL
  simlist$srvrbusy <- F
  # defining job numbers is good practice, always invaluable during
  # debugging
...
nursesreact <- function(evnt,simlist) {
  ...
  {  
    ...
    else if (simlist$ncalls == (simlist$nurses + simlist$q)) #activate a new nurse, block a call
    {
      ...
      # the call is blocked due to queue limit, won't add into queue
    }
    ...
    # because the rate stays at 1/2r which is indep of the state, so we can just generate directly 
